...
def load_args():
    ...
    parser.add_argument('--es', action="store_true", help="activate early stop")
    args = parser.parse_args()
    return args

def init_wandb(model, periperal, timestamp,
        lr=0.0001,
        nl=1,
        dr=0.1,
        dm=32,
    ...
    wandb.init(
        project="New-ML-PROTEL", 
        name=f"{model}_{periperal}_{timestamp}",
        config={
            "learning_rate": lr,
            "architecture": model,
...
def main():
    ...
    if args.wandb : init_wandb(m_type, periperal, timestamp, LR, N_LAYERS, DROPOUT, D_MODEL, N_HEADS, loss_func.__class__.__name__)
    ...
    for e in range(EPOCH):
        ...
        if args.log or e == EPOCH - 1 :
            print(f"[Epoch {e+1}/{EPOCH}] Val Loss : {epoch_val_loss:.4f} | Val Accuracy : {epoch_val_acc:.4f} | LR : {current_lr}")
