...
# Pastikan tsai, helper, dan config tersedia di environment-mu
...
def main():
    ...
    NUM_CLASSES = 2 # Diubah ke 2 Kelas (Awake, Drowsy)
    ##################################
    ...
    # Gunakan data_ready/train_filter.pkl jika kamu sudah mengimplementasikan pembuangan noise
    data = np.load("data_ready/train_filter.npz") 
    X_sec, X_tab, y, groups = data["X_seq"], data["X_tab"], data["y"], data["id"]

    # Filter subjek bermasalah (jika ada)
    # bad_subjects = ['22020', '23051'] 
    # mask = ~np.isin(groups, bad_subjects)
    # X_sec, X_tab, y, groups = X_sec[mask], X_tab[mask], y[mask], groups[mask]
    ...
    for fold, (train_idx, val_idx) in enumerate(logo.split(X_sec, y, groups)):
        ...
        if args.wandb : 
            project_name="LOSO_Evaluation_2_Class"
    ...
    # ROC AUC untuk binary classification hanya memerlukan probabilitas kelas positif (Drowsy)
