...
def main():
    ...
    model = TFTModel(
        input_chunk_length= train_params.get("input_chunk_length", CONFIG.INPUT_CHUNK_LEN), # change to seq_len * context_time
        output_chunk_length= CONFIG.OUTPUT_CHUNK_LEN, # change to seq_len * prediction_time
        hidden_size= train_params.get("hidden_size", CONFIG.HIDDEN_SIZE),
        lstm_layers= train_params.get("lstm_layers", CONFIG.LSTM_LAYERS),
        num_attention_heads= train_params.get("num_attention_heads", CONFIG.ATT_HEADS),
        dropout= train_params.get("dropout", CONFIG.DROPOUT),
        batch_size=CONFIG.BATCH_SIZE, # change to batch_size
        n_epochs= CONFIG.EPOCH, # change to epoch
        optimizer_kwargs={"lr": train_params.get("learning_rate", CONFIG.L_RATE)},
